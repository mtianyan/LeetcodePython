    ...
    def readBinaryWatch(self, num: int) -> List[str]:
        ...
        def traceback(num, index, status):
            ...
            for i in range(index, 10):
                status[i] = 1  # 做出下一步的选择   第i个灯亮了，进行递归
                traceback(num - 1, i + 1, status)  # 在剩下的灯里面去点亮num-1个灯
                status[i] = 0  # 返回到上一步选择时的状态     第i个灯没亮，i+1继续判断
...
class Solution:
    def readBinaryWatch(self, num: int) -> List[str]:
        ...
        result = []
        for n in range(max(num-5,0),min(num+1,6)):
            for hour in [h for h in buf[n] if h <12]:
                for minute in buf[num-n]:
...
class Solution:
    ...
    def readBinaryWatch(self, num: int) -> List[str]:
        ...
        return [f'{hour}:{minute:02d}' for hour in range(12) for minute in range(60) if bitcount(hour)+bitcount(minute) == num]
